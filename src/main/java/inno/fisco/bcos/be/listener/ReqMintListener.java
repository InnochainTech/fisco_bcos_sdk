...
import inno.fisco.bcos.be.entity.response.BatchMintVo;
import inno.fisco.bcos.be.entity.usesign.ReqVo;
import inno.fisco.bcos.be.entity.usesign.request.BatchMintReq;
 ...
 */
@Component
public class ReqMintListener implements MessageListenerConcurrently {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReqMintListener.class);
    ...
    private void mockConsume(String msg) throws ABICodecException, TransactionException, NoSuchTransactionFileException, IOException {
        LOGGER.info("ReqMintListener receive msg: {}.", msg);
        Result<BatchMintVo> result = new Result();

        ReqVo reqVo = JSONObject.parseObject(msg, ReqVo.class);
        BatchMintReq batchMintReq = JSONObject.parseObject(reqVo.getFuncParam().toString(),BatchMintReq.class);
        reqVo.setFuncParam(batchMintReq);
        ...
        if (errorInfos.getErrors().size() != 0) {
            ...
            result = mqnftService.batchMint(reqVo);
        }
        LOGGER.info("ReqBurnListener mockConsume success result:{}.", result.toString());
        String resultStr = JSONObject.toJSONString(result);

        rocketMsgService.simpleSendMsg(MQConstant.CHAIN_RESPONSE_TOPIC, MQConstant.BCOS_RES_NFT_MINT_TAG, resultStr);
    }
}
