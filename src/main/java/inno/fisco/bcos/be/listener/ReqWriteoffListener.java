...
import inno.fisco.bcos.be.entity.response.WriteOffVo;
import inno.fisco.bcos.be.entity.usesign.ReqVo;
import inno.fisco.bcos.be.entity.usesign.request.WriteOffReq;
...
public class ReqWriteoffListener implements MessageListenerConcurrently {
    ...
    private void mockConsume(String msg) throws ABICodecException, TransactionException, NoSuchTransactionFileException, IOException {
        LOGGER.info("ReqWriteoffListener receive msg: {}.", msg);
        Result<WriteOffVo> result = new Result();

        ReqVo reqVo = JSONObject.parseObject(msg, ReqVo.class);
        WriteOffReq writeOffReq = JSONObject.parseObject(reqVo.getFuncParam().toString(),WriteOffReq.class);
        reqVo.setFuncParam(writeOffReq);
        ...
        if (errorInfos.getErrors().size() != 0) {
            ...
            result = mqnftService.writeOff(reqVo);
        }
        LOGGER.info("ReqWriteoffListener mockConsume success result:{}.", result.toString());
