...
import inno.fisco.bcos.be.entity.response.BatchSellVo;
import inno.fisco.bcos.be.entity.usesign.ReqVo;
import inno.fisco.bcos.be.entity.usesign.request.BatchSellReq;
 ...
 */
@Component
public class ReqSellListener implements MessageListenerConcurrently {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReqSellListener.class);
    ...
    private void mockConsume(String msg) throws ABICodecException, TransactionException, NoSuchTransactionFileException, IOException {
        LOGGER.info("ReqSellListener receive msg: {}.", msg);
        Result<BatchSellVo> result = new Result();

        ReqVo reqVo = JSONObject.parseObject(msg, ReqVo.class);
        BatchSellReq batchSellReq = JSONObject.parseObject(reqVo.getFuncParam().toString(),BatchSellReq.class);
        reqVo.setFuncParam(batchSellReq);
        ...
        if (errorInfos.getErrors().size() != 0) {
            ...
            result = mqnftService.batchSell(reqVo);
        }
        LOGGER.info("ReqSellListener mockConsume success result:{}.", result.toString());
        String resultStr = JSONObject.toJSONString(result);
        rocketMsgService.simpleSendMsg(MQConstant.CHAIN_RESPONSE_TOPIC, MQConstant.BCOS_RES_NFT_SELL_TAG, resultStr);
    }
}
