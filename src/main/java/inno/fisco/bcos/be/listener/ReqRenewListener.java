...
import inno.fisco.bcos.be.entity.response.RenewVo;
import inno.fisco.bcos.be.entity.usesign.ReqVo;
import inno.fisco.bcos.be.entity.usesign.request.RenewReq;
 ...
 */
@Component
public class ReqRenewListener implements MessageListenerConcurrently {
    private static final Logger LOGGER = LoggerFactory.getLogger(ReqRenewListener.class);
    ...
    private void mockConsume(String msg) throws ABICodecException, TransactionException, NoSuchTransactionFileException, IOException {
        LOGGER.info("ReqRenewListener receive msg: {}.", msg);
        Result<RenewVo> result = new Result();

        ReqVo reqVo = JSONObject.parseObject(msg, ReqVo.class);
        RenewReq renewReq = JSONObject.parseObject(reqVo.getFuncParam().toString(),RenewReq.class);
        reqVo.setFuncParam(renewReq);
        ...
        if (errorInfos.getErrors().size() != 0) {
            ...
            result = mqnftService.renew(reqVo);
        }
        LOGGER.info("ReqBurnListener mockConsume success result:{}.", result.toString());
        String resultStr = JSONObject.toJSONString(result);
        rocketMsgService.simpleSendMsg(MQConstant.CHAIN_RESPONSE_TOPIC, MQConstant.BCOS_RES_NFT_RENEW_TAG, resultStr);
    }
}
