...
import inno.fisco.bcos.be.entity.response.NFTDeployVo;
import inno.fisco.bcos.be.entity.usesign.ReqVo;
import inno.fisco.bcos.be.entity.usesign.request.NFTDeploy;
 ...
 */
@Component
public class ReqDeployListener implements MessageListenerConcurrently {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReqDeployListener.class);
    ...
    private void mockConsume(String msg) throws ABICodecException, TransactionException, NoSuchTransactionFileException, IOException {
        ...
        Result<NFTDeployVo> result = new Result();

        ReqVo reqVo = JSONObject.parseObject(msg, ReqVo.class);
        NFTDeploy nftDeploy = JSONObject.parseObject(reqVo.getFuncParam().toString(),NFTDeploy.class);
        reqVo.setFuncParam(nftDeploy);
        ...
        if (errorInfos.getErrors().size() != 0) {
            ...
            result = mqnftService.deploy(reqVo);
        }
        LOGGER.info("ReqDeployListener mockConsume success result:{}.", result.toString());
        String resultStr = JSONObject.toJSONString(result);
        rocketMsgService.simpleSendMsg(MQConstant.CHAIN_RESPONSE_TOPIC, MQConstant.BCOS_RES_NFT_DEPLOY_TAG, resultStr);
    }
}
