...
public class LegoTransaction {
    ...
    @Autowired
    public BcosClientWrapper bcosClientWrapper;

    public TransactionPusherInterface txPusher;
    ...
    public TransactionResponse sendTransactionAndGetResponse(
            int chainId, BasicAbiTransaction abiTx, ISignTransaction signTxImpl, String signUserId)
        ...
        String signatureStr =
                signTxImpl.requestForSign(
                        rawTxHash, bcosClientWrapper.getTxCryptoSuite().getCryptoTypeConfig(),signUserId);
        SignatureResult signature = decodeSignatureString(signatureStr);
        // 产生带有签名的交易
        byte[] signedTransaction =
                abiTx.encodeRawTransactionWithSignature(rawTransaction, signature);
        // 发送交易
        txPusher = new TransactionPusherService(bcosClientWrapper.getClient());
